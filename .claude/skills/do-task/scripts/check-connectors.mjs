...
const add = (name, ok, detail, fix) => {
  out.checks.push({ name, ok, detail, fix });
  if (!ok) out.ok = false;
};
  ...
  if (token) {
    const r = await fetch("https://api.github.com/user", {
      headers: {
        ...
        "User-Agent": "tedi-do-task",
      },
    });
    if (r.ok) login = (await r.json()).login;
    const org = await fetch("https://api.github.com/repos/projetotedi/tedi-back", {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: "application/vnd.github+json",
        "User-Agent": "tedi-do-task",
    ...
    add(
      ...
      !!login && org.ok,
      login
        ? `login ${login}; acesso a projetotedi/tedi-back: ${org.status}`
        : `GET /user falhou (${r.status})`,
      "Fazer um `git push` manual uma vez para o Git Credential Manager salvar a credencial da conta com acesso à org projetotedi",
    );
  } else add("github.credential", false, "credential fill sem password", "git push manual uma vez");
} catch (e) {
  add("github.credential", false, String(e.message).split("\n")[0], "git push manual uma vez");
}
...
if (critPath) {
  ...
  add(
    "crit",
    false,
    go.status === 0 ? `crit ausente; ${go.stdout.trim()} disponível` : "crit ausente e Go ausente",
    go.status === 0
      ? "go install github.com/tomasz-tomczyk/crit/cmd/crit@latest  (e garantir a pasta go/bin do usuário no PATH)"
      : "Baixar crit-windows-amd64.exe em https://github.com/tomasz-tomczyk/crit/releases, renomear para crit.exe e colocar no PATH",
  );
}
...
const yarn =
  process.platform === "win32" ? run("cmd", ["/c", "yarn --version"]) : run("yarn", ["--version"]);
add("yarn", yarn.status === 0, yarn.stdout.trim(), "npm i -g yarn");
...
add(
  "repo",
  !!repoRoot && ["tedi-back", "tedi-front"].includes(repoName),
  repoRoot ? `${repoName} em ${repoRoot}` : "não está dentro de um clone git",
  "Rodar a skill a partir da raiz de tedi-back ou tedi-front",
);
if (repoRoot) {
  ...
  out.checks.push({
    ...
    ok: true,
    detail: has ? sp : `ausente em ${sp} (só necessário para cards que tocam os dois repositórios)`,
    fix: has ? undefined : `git clone https://github.com/projetotedi/${sibling} ${sp}`,
  });
}
...
if (process.argv.includes("--json")) console.log(JSON.stringify(out, null, 2));
  ...
  for (const c of out.checks)
    console.log(
      `${c.ok ? "OK " : "FALHA"}  ${c.name.padEnd(20)} ${c.detail}${c.ok || !c.fix ? "" : `\n        → ${c.fix}`}`,
    );
  console.log(
    out.ok
      ? "\nTudo pronto."
