...
const api = async (method, path, body) => {
  const r = await fetch(`https://api.github.com/repos/${owner}/${spec.repo}${path}`, {
    method,
    headers: H,
  ...
  if (!r.ok && !(method === "GET" && r.status === 404))
    throw new Error(
      `${method} ${path} → ${r.status}: ${typeof json === "string" ? json : json.message} ${json.errors ? JSON.stringify(json.errors) : ""}`,
    );
...
if (baseRef.status === 404) {
  if (!spec.createBaseFrom)
    throw new Error(
      `Branch base '${base}' não existe em ${owner}/${spec.repo}. Passe createBaseFrom:"main" para criar, ou mude a base.`,
    );
...
const existing = new Set(
  (await api("GET", "/labels?per_page=100")).json.map((l) => l.name.toLowerCase()),
);
...
const open = (await api("GET", `/pulls?state=open&head=${owner}:${encodeURIComponent(spec.head)}`))
  .json;
let pr;
...
await api("PATCH", `/issues/${pr.number}`, {
