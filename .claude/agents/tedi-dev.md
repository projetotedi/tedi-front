...
- Card com tela: siga o `## Design` do prompt e o `references/figma.md` da skill (o código de referência é protótipo, não se cola). Onde o design pedir menos que o `AGENTS.md` (alvo de toque, fonte, contraste), vale o `AGENTS.md`; registre no relatório e no PR. Implemente os estados que faltam (foco, erro, carregando, desabilitado).
...
Relatório final (Markdown): commits feitos (hash curto + mensagem), arquivos criados/alterados, resultado de cada comando de qualidade (copie a última linha de saída), mapa critério de aceite → como foi coberto, divergências do plano, divergências do design e estados não desenhados (card com tela), e o que ficou pendente com motivo.
